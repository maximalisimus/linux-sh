...
function trnsltlng()
{
	...
	for i in ${_lng[*]}; do
		...
		if [ -e $_out_file ]; then
			_lf=$(cat $_lang_file | awk '!/^$/{print $0}' | sed 's/[ \t]//g' | wc -l)
			_rf=$(cat $_out_file | awk '!/^$/{print $0}' | sed 's/[ \t]//g' | wc -l)
			if [[ $_lf -ne $_rf ]]; then
				echo -e -n "\e[1;31mError translate ${_lng_files[$count]} = $i !\e[0m"
				outin_failure
				wait
				if [[ $cnt -eq 2 ]]; then
					cnt=1
					if [[ $_is_srv -eq 2 ]]; then
						_is_srv=1
					else
						let _is_srv+=1
					fi
				...
				echo "" > $_error_file
				translatetext "$_sever" "$i" "$_lang_file" "$_out_file" 2>$_error_file
				wait
				_err_log=$(cat $_error_file | awk '!/^$/{print $0}' | sed 's/[ \t]//g' | grep -vi "did")
				if [[ $_err_log != "" ]]; then
					echo -e -n "\e[1;31mError translate ${_lng_files[$count]} = $i !\e[0m"
					outin_failure
					flashline
