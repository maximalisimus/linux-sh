...
sms()
{
	case "$1" in
		1) case "$2" in
				1) echo -e -n "PKGBUILD2 to complete";; 
				2) echo -e -n "PKGBUILD2 to failure";;
			esac;;
	esac
}
question()
...
echo -e "\e[1;30m\n\nStart $0\e[0m"
echo ""
cat PKGBUILD 2> /dev/null | while read line
do
	if [[ ${line} =~ ^"arch=(" ]]; then
		echo "arch=('any')" >> PKGBUILD2
...
question "1"
#echo -e "\n"
unset nstr 2> /dev/null 1> /dev/null
unset nstrs 2> /dev/null 1> /dev/null
