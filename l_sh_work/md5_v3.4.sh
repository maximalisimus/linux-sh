...
md5_program()
{
	...
	if [[ $_md5sum_check != "" ]]; then 
		if [[ $_maxdepth = "" ]]; then
			find $_folder -type f | grep -v "$0" | grep -v "$_md5" | grep -v "$_error_md5" | grep -v "$_error_test_md5" | grep -v "$_md5sum_check" | xargs md5sum 1>>$_md5 2>>$_error_md5
		else
			find $_folder -maxdepth "$_maxdepth" -type f | grep -v "$0" | grep -v "$_md5" | grep -v "$_error_md5" | grep -v "$_error_test_md5" | grep -v "$_md5sum_check" | xargs md5sum 1>>$_md5 2>>$_error_md5
		fi
	else
		if [[ $_maxdepth = "" ]]; then
			find $_folder -type f | grep -v "$0" | grep -v "$_md5" | grep -v "$_error_md5" | grep -v "$_error_test_md5" | xargs md5sum 1>>$_md5 2>>$_error_md5
		else
			find $_folder -maxdepth "$_maxdepth" -type f | grep -v "$0" | grep -v "$_md5" | grep -v "$_error_md5" | grep -v "$_error_test_md5" | xargs md5sum 1>>$_md5 2>>$_error_md5
		fi
	fi
	wait
